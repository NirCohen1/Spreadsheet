...
class SharableSpreadsheet
{
    ...
    public SharableSpreadsheet(int nRows, int nCols)
    {
        ...
        saving = new Mutex();
        searchers = new SemaphoreSlim(m_searchers, m_searchers);
    ...
    public bool setConcurrentSearchLimit(int nUsers)
    {
        ...
        if (searchers.CurrentCount >= nUsers)
            return false;

        for (int i = 0; i < m_searchers; i++)
            searchers.Wait();

        m_searchers = nUsers;
        searchers = new SemaphoreSlim(nUsers, nUsers);
